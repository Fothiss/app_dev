...
class OrderService:
    ...
    async def list_orders(self, session: Session, count: int = 10, page: int = 1, user_id: Optional[int] = None) -> List[Order]:
        return await self.order_repository.list(session, count=count, page=page, user_id=user_id)

    async def update_status(self, session: Session, order_id: int, status: str) -> Optional[Order]:

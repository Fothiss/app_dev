...
class TestOrderService:
    def test_create_order(self, session, order_service: OrderService):
        async def _run():
            ...
            address = Address(user_id=user.id, street="Street 1", city="City 1", country="Country 1")
            session.add(address)
            session.commit()
            session.refresh(address)

            product = Product(name="Service Product 1", price=Decimal("50.00"), stock_quantity=10)
            session.add(product)
            session.commit()
    ...
    def test_list_orders(self, session, order_service: OrderService):
        async def _run():
            ...
            address = Address(user_id=user.id, street="Street 2", city="City 2", country="Country 2")
            session.add(address)
            session.commit()
            session.refresh(address)

            product = Product(name="Service Product 2", price=Decimal("25.00"), stock_quantity=20)
            session.add(product)
            session.commit()
            session.refresh(product)
    ...
    def test_update_order_status(self, session, order_service: OrderService):
        async def _run():
            ...
            address = Address(user_id=user.id, street="Street 3", city="City 3", country="Country 3")
            session.add(address)
            session.commit()
            session.refresh(address)

            product = Product(name="Service Product 3", price=Decimal("30.00"), stock_quantity=15)
            session.add(product)
            session.commit()
            session.refresh(product)
    ...
    def test_delete_order(self, session, order_service: OrderService):
        async def _run():
            ...
            address = Address(user_id=user.id, street="Street 4", city="City 4", country="Country 4")
            session.add(address)
            session.commit()
            session.refresh(address)

            product = Product(name="Service Product 4", price=Decimal("40.00"), stock_quantity=5)
            session.add(product)
            session.commit()
            session.refresh(product)

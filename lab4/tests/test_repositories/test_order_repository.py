...
class TestOrderRepository:
    def test_create_order(self, session, order_repository: OrderRepository):
        async def _run():
            ...
            # Создаем продукт
            product = Product(name="Order Product 1", price=Decimal("50.00"), stock_quantity=10)
            session.add(product)
            session.commit()
    ...
    def test_list_orders(self, session, order_repository: OrderRepository):
        async def _run():
            ...
            address1 = Address(user_id=user1.id, street="Street 2a", city="City 2a", country="Country 2a")
            ...
            address2 = Address(user_id=user2.id, street="Street 2b", city="City 2b", country="Country 2b")
            session.add(address2)
            session.commit()
            session.refresh(address2)

            # Создаем продукт
            product = Product(name="Order Product 2", price=Decimal("25.00"), stock_quantity=20)
            session.add(product)
            session.commit()
    ...
    def test_update_order_status(self, session, order_repository: OrderRepository):
        async def _run():
            ...
            # Создаем продукт
            product = Product(name="Order Product 3", price=Decimal("30.00"), stock_quantity=15)
            session.add(product)
            session.commit()
            session.refresh(product)
            ...
            updated = await order_repository.update_status(session, order.id, "completed")

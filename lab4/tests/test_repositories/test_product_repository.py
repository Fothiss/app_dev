...
class TestProductRepository:
    def test_create_product(self, session, product_repository: ProductRepository):
        async def _run():
            product = await product_repository.create(
                session,
                ProductCreate(name="Test Product 1", price=Decimal("99.99"), stock_quantity=5),
            )
    ...
    def test_get_list_products(self, session, product_repository: ProductRepository):
        async def _run():
            await product_repository.create(
                session,
                ProductCreate(name="Test Product 2a", price=Decimal("50.00"), stock_quantity=2),
            )
            await product_repository.create(
                session,
                ProductCreate(name="Test Product 2b", price=Decimal("75.00"), stock_quantity=4),
            )
    ...
    def test_update_product(self, session, product_repository: ProductRepository):
        async def _run():
            product = await product_repository.create(
                session,
                ProductCreate(name="Test Product 3", price=Decimal("20.00"), stock_quantity=1),
            )
    ...
    def test_update_stock(self, session, product_repository: ProductRepository):
        async def _run():
            product = await product_repository.create(
                session,
                ProductCreate(name="Test Product 4", price=Decimal("10.00"), stock_quantity=10),
            )
    ...
    def test_delete_product(self, session, product_repository: ProductRepository):
        async def _run():
            product = await product_repository.create(
                session,
                ProductCreate(name="Test Product 5", price=Decimal("30.00"), stock_quantity=1),
            )

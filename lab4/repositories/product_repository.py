...
class ProductRepository:
    ...
    async def get_list(self, session: Session, count: int = 10, page: int = 1, **filters) -> List[Product]:
    ...
    async def update(self, session: Session, product_id: int, product_data: ProductUpdate) -> Optional[Product]:
    ...
    async def update_stock(self, session: Session, product_id: int, quantity_change: int) -> Optional[Product]:
        product = session.query(self.model).filter(self.model.id == product_id).first()
        if not product:
            return None
